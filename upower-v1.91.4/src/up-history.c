...
up_history_copy_array_timespan (const GPtrArray *array, guint timespan)
{
	...
	/* new data */
	array_new = g_ptr_array_new_with_free_func ((GDestroyNotify) g_object_unref);
...
up_history_array_to_file (UpHistory *history, GPtrArray *list, const gchar *filename)
{
	...
	/* save to disk */
	ret = g_file_set_contents_full (filename, part, -1, G_FILE_SET_CONTENTS_CONSISTENT, 0640, &error);
	if (!ret) {
